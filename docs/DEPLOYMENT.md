...
7. **Backups and Retention**: Back up the SQLite database if task state, verified cache entries, attempt history, or API aggregates matter operationally. Configure `security.retention_days` to keep trace and telemetry volume aligned with your data-retention policy.
